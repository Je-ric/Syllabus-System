...
Single source of truth for understanding the full system: what it does, who can do what, and critical rules to remember.
...
- `OtpService::issueForUser()` returns `bool` — mail failure is logged but never crashes the app.
...
- **Cannot delete syllabi** — deletion is disabled to protect academic records
...
5. User is redirected to the OTP verification page (`/show-otp?email=...`).

### OTP Verification

- Email is passed via URL query param (`?email=...`) AND stored in session via `$request->session()->put()`.
- Wrong OTP → stays on OTP page with error, does NOT redirect away.
- Non-numeric input → stays on OTP page with validation error.
...
- Email not verified → warning toast, stays on auth page.
...
```
College
  └── Dean (1 per college, assigned by admin only)
  └── Department
        └── Chair (1 per department, assigned by admin only)
        └── Faculty (multiple per department, assigned by admin only)
```
...
- Only the preparer can edit their own syllabus.
- Syllabi **cannot be deleted** by faculty — deletion is disabled to protect academic records.
...
- OTP session uses `$request->session()->put('verify_email', $email)` (persistent, not flash) so it survives refreshes and wrong-OTP retries.
...
| `/organizational/hierarchy`, `/organizational/college/*/departments` | `role:admin,dean,chair` | Admin, Dean, Chair |
...
| Topic | File |
|---|---|
| Authentication & OTP | `01_Authentication_OTP_Account_Status.md` |
| Academic Calendar | `02_Academic_Calendar_and_Events.md` |
| Academic Structure | `03_Academic_Structure.md` |
| Course Management | `04_Course_Management.md` |
| Goals & Objectives | `05_Goals_and_Objectives.md` |
| PEOs & POs | `06_Program_PEO_PO.md` |
| Org Hierarchy | `07_Organizational_Hierarchy.md` |
| OTP Flow | `08_OTP_Flow_and_Service.md` |
| Roles & Assignments | `09_Roles_Assignment.md` |
| Syllabus Wizard | `10_Syllabus_Wizard.md` |
| Program Selector | `11_Program_Selector.md` |
| Audit Logs | `12_Audit_Logs.md` |
| Syllabus CRUD | `14_Syllabus_CRUD.md` |
| User Profile | `15_User_Profile.md` |
