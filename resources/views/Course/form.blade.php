...
<form action="{{ $formAction }}" method="POST">
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <input type="text" name="code" class="border rounded px-2 py-1 w-full" value="{{ old('code', $course->course_code ?? '') }}" required>
            @error('code')
        ...
        <div>
            ...
            <input type="text" name="name" class="border rounded px-2 py-1 w-full" value="{{ old('name', $course->course_title ?? '') }}" required>
            @error('name')
                <span class="text-red-600 text-sm">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div class="mb-4">
        ...
        <textarea name="description" class="border rounded px-2 py-1 w-full" rows="3">{{ old('description', $course->course_description ?? '') }}</textarea>
        @error('description')
            <span class="text-red-600 text-sm">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-4">
        <div>
            ...
            <input type="number" name="credits" class="border rounded px-2 py-1 w-full" value="{{ old('credits', $course->credit_units ?? '') }}" min="0" required>
            @error('credits')
                <span class="text-red-600 text-sm">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <input type="text" name="prerequisite" class="border rounded px-2 py-1 w-full" value="{{ old('prerequisite', $course->prerequisite ?? '') }}">
        </div>
        <div>
            <label class="block font-semibold">Corequisite</label>
            <input type="text" name="corequisite" class="border rounded px-2 py-1 w-full" value="{{ old('corequisite', $course->corequisite ?? '') }}">
        </div>
    </div>
