...
<head>
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        ink:      '#003a10',  // clsu green-150 — deepest ink
                        forest:   '#1a5f30',  // clsu-cobra — primary dark green
                        moss:     '#009639',  // clsu-green — brand accent
                        parchment:'#f1f2f3',  // clsu white-50
                        paper:    '#f7f8f7',  // near-white canvas
                        gold:     '#e0a70d',  // clsu-gold
                        'gold-ink':'#a6790a', // darkened clsu-gold for text-on-light
                        slate:    '#525252',  // clsu black-20
                    },
                    fontFamily: {
    ...
    <style>
        ...
        /* Oswald reads best slightly opened up at small sizes and slightly tightened at display sizes */
        ...
        /* Registry stamp — sharper corners than a SaaS pill, reads as an actual rubber stamp / file marker */
        .stamp {
            display: inline-flex;
            align-items: center;
            gap: 0.4rem;
        ...
        .stamp-approved { color: #009639; }
        .stamp-review   { color: #a6790a; }
        .stamp-draft    { color: #525252; }

        .doc-card {
            background: #ffffff;
            border: 1px solid rgba(26,95,48,0.14);
            box-shadow: 0 1px 0 rgba(26,95,48,0.05), 0 18px 40px -20px rgba(26,95,48,0.24);
        }
        ...
        /* Folder-tab nav item — a registry-cabinet motif, not a rounded SaaS nav pill */
        .tab-link {
            position: relative;
            padding-bottom: 2px;
        }
        ...
        /* Index number — used on the workflow steps in place of generic circular icon badges */
        .index-tab {
            font-family: 'Oswald', sans-serif;
            font-weight: 700;
            font-size: 13px;
...
<body class="font-sans text-ink antialiased">
    ...
    <nav class="fixed top-0 inset-x-0 z-50 bg-paper/90 backdrop-blur-md border-b border-ink/10">
        <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-8 text-sm font-medium text-slate">
                <a href="#about" class="tab-link hover:text-ink">What it does</a>
            ...
            <a href="{{ route('auth.show') }}"
               class="inline-flex items-center gap-2 bg-forest text-paper font-semibold text-sm px-5 py-2.5 rounded-sm hover:bg-ink transition-colors">
                Sign in
    ...
    <section id="top" class="grain relative pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden bg-white">
        <div class="max-w-6xl mx-auto px-6 grid lg:grid-cols-[1.05fr_0.95fr] gap-16 items-center">
            ...
            <div>
                <p class="eyebrow text-xs text-gold-ink mb-5">
                ...
                <div class="flex flex-wrap items-center gap-4 mb-10">
                    <a href="{{ route('auth.show') }}"
                       class="inline-flex items-center gap-2 bg-forest text-paper font-semibold px-6 py-3.5 rounded-sm hover:bg-ink transition-colors text-base shadow-lg shadow-forest/20">
                        Sign in to CSMS
                    </a>
                    <a href="#workflow"
                       class="inline-flex items-center gap-2 text-ink font-semibold px-2 py-3.5 text-base tab-link">
            ...
            <div class="relative h-[420px] hidden sm:block" aria-hidden="true">
                <div class="doc-card ruled-line absolute inset-x-0 top-0 h-[320px] rounded-sm p-6 flex flex-col">
                    ...
                    <div class="space-y-2 flex-1">
                        <div class="h-2 rounded-sm bg-ink/10 w-[85%]"></div>
                        <div class="h-2 rounded-sm bg-ink/10 w-[70%]"></div>
                        <div class="h-2 rounded-sm bg-ink/10 w-[92%]"></div>
                        <div class="h-2 rounded-sm bg-ink/10 w-[60%]"></div>
                    </div>
    ...
    <section id="about" class="py-24 bg-white border-t border-ink/10">
        <div class="max-w-6xl mx-auto px-6">
            <div class="max-w-xl mb-16">
                <p class="eyebrow text-xs text-gold-ink mb-3">What it does</p>
                <h2 class="ledger-head text-ink text-3xl md:text-4xl leading-tight mb-4">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                @foreach([
                    ['01', 'bx-edit-alt', 'Build the syllabus', 'Faculty complete a step-by-step wizard covering course outcomes, weekly coverage, and grading weights. The system enforces completeness — a syllabus cannot be submitted until all required sections are filled.'],
                    ['02', 'bx-message-detail', 'Chair review', 'Submitted syllabi go to the department chair first. The chair can return it with feedback or endorse it to the dean — keeping incomplete work from reaching final approval.'],
                    ['03', 'bx-badge-check', 'Dean approval', 'The dean gives final sign-off for the college. Once approved, the syllabus is locked and stored — no more confusion over which version was the final one.'],
                ] as $i => $f)
                <div class="doc-card rounded-sm p-7">
                    <div class="flex items-center justify-between mb-6">
                        <div class="w-11 h-11 rounded-sm bg-forest/10 flex items-center justify-center">
                            <i class="bx {{ $f[1] }} text-xl text-forest" aria-hidden="true"></i>
                        </div>
                        <span class="index-tab text-gold-ink">{{ $f[0] }}</span>
                    </div>
                    <h3 class="ledger-head text-ink text-xl mb-2.5">{{ $f[2] }}</h3>
                    <p class="text-slate text-[0.95rem] leading-relaxed">{{ $f[3] }}</p>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <section id="workflow" class="py-24 bg-forest text-paper">
        <div class="max-w-6xl mx-auto px-6">
            <div class="max-w-xl mb-20">
                ...
                <h2 class="ledger-head text-3xl md:text-4xl leading-tight mb-4">
            ...
            <div class="grid md:grid-cols-4 gap-x-6 gap-y-12 relative">
                @foreach([
                    ['bx-user-plus', 'Register', 'Sign up with your CLSU email, then wait for an admin to confirm your account before you can log in.', 'draft'],
                ...
                <div class="relative border-l-2 border-paper/15 pl-5">
                    <span class="index-tab text-gold block mb-3">0{{ $i + 1 }}</span>
                    <div class="w-10 h-10 rounded-sm bg-paper/10 border border-paper/25 flex items-center justify-center mb-4">
    ...
    <section id="roles" class="py-24 bg-parchment">
        <div class="max-w-6xl mx-auto px-6">
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div class="bg-white border border-ink/10 rounded-sm p-6 hover:-translate-y-1 hover:shadow-lg hover:shadow-ink/5 transition-all duration-200">
    ...
    <section class="py-20 bg-white border-y border-ink/10">
        <div class="max-w-6xl mx-auto px-6 grid md:grid-cols-3 gap-8">
            @foreach([
                ...
                ['bx-user-check', 'Admin review', 'Your registration is reviewed by an admin before access is granted to your account.'],
            ...
            <div class="flex gap-4">
                <div class="w-10 h-10 rounded-sm bg-forest/10 flex items-center justify-center shrink-0">
    ...
    <section class="grain relative py-24 bg-ink text-paper overflow-hidden">
        <div class="max-w-2xl mx-auto px-6 text-center relative">
            ...
            <h2 class="ledger-head text-3xl md:text-4xl mb-4">
                Sign in with your CLSU email.
            </h2>
            <p class="text-paper/70 mb-9 leading-relaxed max-w-md mx-auto">
                New to CSMS? Registration is on the same page — sign up with your CLSU email, then an admin will approve your account before you get access.
            </p>
            <a href="{{ route('auth.show') }}"
               class="inline-flex items-center gap-2 bg-gold text-ink font-semibold px-8 py-4 rounded-sm hover:bg-paper transition-colors text-base shadow-lg">
                Go to sign-in
        ...
        <div class="max-w-6xl mx-auto px-6">
            <div class="flex flex-col md:flex-row justify-between gap-10 mb-10">
                ...
                <nav class="flex gap-8 text-sm" aria-label="Footer">
                    <a href="#about" class="tab-link hover:text-paper">What it does</a>
