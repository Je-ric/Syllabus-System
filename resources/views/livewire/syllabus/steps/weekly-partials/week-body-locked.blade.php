...
<div class="rounded-xl border px-4 py-4 mb-3
    ...
    <div class="flex items-start gap-3">
        ...
        <span class="shrink-0 flex items-center justify-center w-9 h-9 rounded-xl
            {{ $isExam ? 'bg-amber-100 text-amber-600' : 'bg-rose-100 text-rose-500' }}">
                ...
                <ul class="mt-3 space-y-1.5">
                    @foreach ($lockingEvents as $ev)
                        <li class="flex items-center gap-2 text-sm
                            ...
                            <span class="opacity-70 text-xs">{{ $ev['date_display'] }}</span>
                        </li>
                    @endforeach
                </ul>
            @endif
...
@if (count($otherEvents) > 0)
    <div class="rounded-xl border border-slate-200 bg-white px-4 py-3.5">
        ...
        <p class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2.5 flex items-center gap-1.5">
            <i class="bx bx-calendar-event text-slate-300 text-sm"></i>
            Other events this week
        </p>
