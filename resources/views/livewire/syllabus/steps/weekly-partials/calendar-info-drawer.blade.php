    ...
    @if ($syllabus?->academicCalendar)
        ...
        <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 mb-4">
            ...
            @if ($cal->academic_year ?? null)
                <p class="text-[11px] text-slate-400 mt-0.5">{{ $cal->academic_year }}</p>
            @endif
        </div>
        ...
        @if ($weeksGenerated)
            ...
            <div class="grid grid-cols-3 gap-2 mb-4">
                <div class="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-center">
                    <p class="text-[18px] font-bold text-emerald-700">{{ $syllabusWeeks->count() }}</p>
                    <p class="text-[10px] text-emerald-600 font-semibold uppercase tracking-wide">Weeks</p>
                </div>
                <div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-center">
                    <p class="text-[18px] font-bold text-slate-700">{{ collect($weekEvents)->flatten(1)->count() }}</p>
                    <p class="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">Events</p>
                </div>
                <div class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-center">
                    <p class="text-[18px] font-bold text-amber-700">{{ $lockedCount }}</p>
                    <p class="text-[10px] text-amber-600 font-semibold uppercase tracking-wide">Locked</p>
                </div>
            </div>
        @endif
        ...
        @foreach ($drawerGrouped as $type => $meta)
            ...
            @if (count($typeEvents) > 0)
                ...
                <div class="mb-4">
                    <p class="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 mb-2">{{ $meta['label'] }}</p>
                        ...
                        @foreach ($typeEvents as $ev)
                            <div class="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-[13px] font-medium text-slate-700">{{ $ev['name'] }}</p>
                                    <p class="text-[11px] text-slate-400">{{ $ev['date_display'] }}</p>
                                </div>
                                <span class="shrink-0 text-[11px] font-semibold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">Wk {{ $ev['week_no'] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
