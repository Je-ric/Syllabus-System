...
<dialog id="purgeAuditLogsModal" class="modal" {{ $confirmingPurge ? '' : 'style="display:none"' }}>
    <div class="modal-box w-11/12 max-w-md max-h-[90vh] p-0 overflow-hidden rounded-2xl bg-white flex flex-col border border-[#E3E8EB] relative" style="min-width: min(540px, 94vw); box-shadow:0 1px 2px rgba(16,24,40,0.04),0 4px 16px rgba(229,47,40,0.12),0 12px 40px rgba(229,47,40,0.10);">
        ...
        <header class="px-5 py-4 border-b border-[#E3E8EB] bg-white shrink-0">
            <div class="flex items-center justify-between gap-4">
                ...
                <button type="button" onclick="document.getElementById('purgeAuditLogsModal').close()" class="shrink-0 rounded-lg p-1.5 text-[#A5B2BD] hover:text-[#394056] hover:bg-[#F1F3F5] active:scale-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00C075]/30 transition-all duration-150" aria-label="Close">
        ...
        <footer class="border-t border-[#F1F3F5] bg-[#F9FAFA] px-5 py-3.5 flex gap-3 shrink-0 justify-end">
            <button type="button" onclick="document.getElementById('purgeAuditLogsModal').close()"
...
document.addEventListener('DOMContentLoaded', function() {
    const modal = document.getElementById('purgeAuditLogsModal');
    const shouldOpen = {{ $confirmingPurge ? 'true' : 'false' }};
    if (modal && shouldOpen) {
        modal.showModal();
    }
});
